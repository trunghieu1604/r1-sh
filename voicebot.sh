...
grant_permissions() {
    local pkg="$1"
    log_info "Cap quyen (neu he thong cho phep)..."

    adb_exec shell pm grant "$pkg" android.permission.RECORD_AUDIO 2>/dev/null || true
    adb_exec shell pm grant "$pkg" android.permission.INTERNET 2>/dev/null || true
    adb_exec shell pm grant "$pkg" android.permission.ACCESS_NETWORK_STATE 2>/dev/null || true
    adb_exec shell pm grant "$pkg" android.permission.ACCESS_WIFI_STATE 2>/dev/null || true
    adb_exec shell pm grant "$pkg" android.permission.CHANGE_WIFI_STATE 2>/dev/null || true
}
